...
public class OpcProsysController {
    org.eclipse.milo.opcua.stack.core.security.SecurityPolicy getSecurityPolicy() {
        return SecurityPolicy.Basic128Rsa15;
    }

    public OpcUaClient getClient() throws  Exception{

        Path securityTempDir = Paths.get("D:/temp/opcua2", "client", "security");
        ...
        List<EndpointDescription> endpoints =
                DiscoveryClient.getEndpoints("opc.tcp://LAPTOP-STJD8A4N:53530/OPCUA/SimulationServer").get();
    ...
    public void readNodeValue() throws Exception{
        ...
        OpcUaClientService opcUaClientService = new OpcUaClientService();
        // 遍历所有节点
        NodeId nodeId = NodeId.parse("ns=3;i=1010");
    ...
    public  void historyData() throws Exception{
        ...
        client.connect().get();
        OpcUaClientService opcUaClientService = new OpcUaClientService();
        System.out.println("--"+new DateTime(DateTime.nowMillis().getUtcTime()- 10*(10 * 1000 * 1000L)));
        System.out.println("++"+new DateTime(DateTime.nowMillis().getUtcTime()));
        HistoryReadDetails historyReadDetails = new ReadRawModifiedDetails(
                false,
                new DateTime(DateTime.nowMillis().getUtcTime()- 10*(10 * 1000 * 1000L)),
                DateTime.nowMillis(),
                uint(0),
                true
        );
        ...
        if (historyReadResults != null) {
            ...
            if (statusCode.isGood()) {
                ...
                dataValues.forEach(v -> System.out.println( v.getServerTime() +",value=" + v.getValue().getValue()));
            } else {
    ...
    public static void main(String[] args){
        try {
            ...
            //new OpcProsysController().listNodes();
            //new OpcProsysController().readNodeValue();
            //new OpcProsysController().writeNodeValue();
            //new OpcProsysController().subscribe2();
            /*
            Double aDouble = new OpcProsysController().callMethod().get();
            System.out.println(aDouble);
            */
            new OpcProsysController().historyData();
